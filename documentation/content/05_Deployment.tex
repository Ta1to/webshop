...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{resources/images/AblaufCICD.png}
...
Für die Auslieferung ist vor allem das manuelle Chunk-Splitting relevant:

\begin{lstlisting}[language=JavaScript, caption=Ausschnitt aus der Vite-Konfiguration (manualChunks), label=lst:vite-config]
build: {
  rollupOptions: {
    output: {
      manualChunks: {
...
Die Trennung von Firebase- und Vue-Abhängigkeiten in eigene Vendor-Chunks verbessert die Cache-Effizienz: Bei Updates müssen primär veränderte Applikations-Chunks neu geladen werden \cite{nguyen2024vite}. Zusätzlich reduziert Tree-Shaking ungenutzten Code im Bundle \cite{vite2025}.
...
Dieser Ansatz reduziert manuelle Eingriffe auf ein Minimum und erhöht gleichzeitig die Zuverlässigkeit jeder Softwareauslieferung. Die Automatisierung des Build-, Test- und Deployment-Zyklus ist ein zentraler Hebel zur Steigerung der Softwarezuverlässigkeit, da sie menschliche Fehlerquellen systematisch ausschließt und konsistente, nachvollziehbare Auslieferungsprozesse garantiert \cite{ugwueze2024, patel2025}.
