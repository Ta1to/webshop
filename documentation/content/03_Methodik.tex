...
Die Sicherheit der Anwendung basiert auf einem mehrschichtigen Ansatz (\textit{Defense in Depth}) \cite{stytz2004}. Die \textit{Authentifizierung} erfolgt über Firebase Authentication mittels JWT-Tokens. Auf der Autorisierungsebene regeln Firestore Security Rules serverseitig, welche Benutzer auf welche Daten zugreifen dürfen \cite{firestore2025}. Da Firebase-Projekte ohne eigene Applikationsschicht direkt aus dem Browser auf Firestore zugreifen, bilden diese Rules die einzige Zugriffskontrollschranke zwischen Client und Datenbank. Ergänzend erfolgt die gesamte Kommunikation über HTTPS, und Firebase verschlüsselt alle gespeicherten Daten automatisch. Benutzereingaben werden zusätzlich clientseitig sowie durch die Security Rules im Backend validiert \cite{fredj2020}. Listing~\ref{lst:security-rules-konzept} zeigt exemplarisch, wie das Prinzip der minimalen Rechtevergabe auf Collection-Ebene umgesetzt ist:

\begin{lstlisting}[language={}, morekeywords={match,allow,read,write,if,true,return,function}, caption={Firestore Security Rules -- Auszug aus firestore.rules}, label=lst:security-rules-konzept]
match /users/{userId} {
...
Nutzerdokumente sind damit strikt auf den jeweiligen Kontoinhaber beschränkt, während Produktdaten zwar öffentlich lesbar, Schreiboperationen jedoch ausschließlich Administratoren vorbehalten sind. Die vollständige Regelstruktur einschließlich der zugehörigen Hilfsfunktionen wird in Kapitel~\ref{ch:testing} im Rahmen der Sicherheitsverifikation detailliert behandelt.